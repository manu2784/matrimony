  ...
  return async function (req, res, next) {
    ...
    if (resource === "user") {
      ...
      if (req.body._id && req.body._id === req.user._id) {
        let allFieldsAllowed = fieldsToUpdate.every((field) =>
          resourcePrevileges.user.user.includes(field),
        );
      ...
      if (req.user.role === 4) {
        let allFieldsAllowed = fieldsToUpdate.every((field) =>
          resourcePrevileges.user.teamAdmin.includes(field),
        );
      ...
      if (req.user.role === 3) {
        let allFieldsAllowed = fieldsToUpdate.every((field) =>
          resourcePrevileges.user.companyAdmin.includes(field),
        );
        if (allFieldsAllowed && targetUser.org === req.user.org) return next();
      }
      ...
      if (req.user.role === 2) {
        let allFieldsAllowed = fieldsToUpdate.every((field) =>
          resourcePrevileges.user.accountManager.includes(field),
        );
